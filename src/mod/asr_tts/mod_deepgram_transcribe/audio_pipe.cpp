...
AudioPipe::AudioPipe(const char* uuid, const char* host, unsigned int port, const char* path,
  ...
  m_state(LWS_CLIENT_IDLE), m_sslFlags(useSSL ? LCCSCF_USE_SSL : 0), m_wsi(nullptr), m_vhd(nullptr),
  m_apiKey(apiKey), m_callback(callback) {
...
bool AudioPipe::connect_client(struct lws_per_vhost_data *vhd) {
  ...
  i.host = i.address;
  i.origin = i.address;
  // RTC: was hardcoded LCCSCF_USE_SSL. Deepgram's real SaaS endpoint
  // (api.deepgram.com:443, the default) always needs TLS; our self-hosted
  // engine's internal NLB (RTC-12670) serves plain ws:// on 8080, VPC/SG-
  // restricted instead, so m_sslFlags is 0 there. See DEEPGRAM_HOST/_PORT/
  // _USE_SSL in dg_transcribe_glue.cpp's fork_data_init().
