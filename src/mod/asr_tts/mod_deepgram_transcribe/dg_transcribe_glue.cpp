...
namespace {
  ...
  switch_status_t fork_data_init(private_t *tech_pvt, switch_core_session_t *session, 
    ...
    // RTC: DEEPGRAM_HOST/_PORT/_USE_SSL override the real Deepgram SaaS
    // default, so a channel can be pointed at a self-hosted engine instead
    // (RTC-12670 - the AWS VoiceFabric self-hosted Deepgram, internal NLB,
    // plain ws:// - no TLS). Unset -> unchanged upstream behavior.
    const char* dgHost = switch_channel_get_variable(channel, "DEEPGRAM_HOST");
    strncpy(tech_pvt->host, dgHost ? dgHost : "api.deepgram.com", MAX_WS_URL_LEN);

    const char* dgPort = switch_channel_get_variable(channel, "DEEPGRAM_PORT");
    tech_pvt->port = dgPort ? (unsigned int) ::atoi(dgPort) : 443;

    const char* dgUseSslVar = switch_channel_get_variable(channel, "DEEPGRAM_USE_SSL");
    bool dgUseSsl = dgUseSslVar ? switch_true(dgUseSslVar) : true;
    ...
    deepgram::AudioPipe* ap = new deepgram::AudioPipe(tech_pvt->sessionId, tech_pvt->host, tech_pvt->port, tech_pvt->path,
      buflen, read_impl.decoded_bytes_per_packet, apiKey, eventCallback, dgUseSsl);
    if (!ap) {
